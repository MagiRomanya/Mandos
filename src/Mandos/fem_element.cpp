                                ...
                                const Vec3& x4)
{
    ...
    Mat3 F;
    F << F_vec(0), F_vec(3), F_vec(6), F_vec(1), F_vec(4), F_vec(7), F_vec(2), F_vec(5), F_vec(8);
    return F;
}
...
Mat9 FEM_LinearMaterial::get_phi_hessian(const Mat3& F) const
{
    ...
    dvecSigma_dvecF << 2 * mu + lambda, 0, 0, 0, lambda, 0, 0, 0, lambda, 0, mu, 0, mu, 0, 0, 0, 0, 0, 0, 0, mu, 0, 0,
        0, mu, 0, 0, 0, mu, 0, mu, 0, 0, 0, 0, 0, lambda, 0, 0, 0, 2 * mu + lambda, 0, 0, 0, lambda, 0, 0, 0, 0, 0, mu,
        0, mu, 0, 0, 0, mu, 0, 0, 0, mu, 0, 0, 0, 0, 0, 0, 0, mu, 0, mu, 0, lambda, 0, 0, 0, lambda, 0, 0, 0,
        2 * mu + lambda;
...
FEM_NeoHookeanMaterial::FEM_NeoHookeanMaterial(Scalar mu_lame, Scalar lambda_lame)
    ...
    : mu(1.33333333333333f * mu_lame)
    , lambda(lambda_lame + 0.833333333333333f * mu_lame)
{
...
Scalar FEM_NeoHookeanMaterial::get_phi(const Mat3& F) const
{
    ...
    const Scalar phi =
        0.5f * mu * (IC - 3.0f) + 0.5f * lambda * (J - alpha) * (J - alpha) - 0.5f * mu * std::log(IC + 1.0f);
    return phi;
}
...
inline Mat9 compute_vectorized_volume_hessian(const Mat3& F)
{
    ...
    Mat9 vec_H;
    vec_H << Mat3::Zero(), -skew(F.col(2)), skew(F.col(1)), skew(F.col(2)), Mat3::Zero(), -skew(F.col(0)),
...
FEM_Element3D<T>::FEM_Element3D(Particle p1,
                                Particle p2,
                                Particle p3,
                                Particle p4,
    ...
    , p4(p4)
    , material(material)
    , dvecF_dx(block_matrix<3, 4>(ds_dx.transpose()))
{
}
...
template <FEM_Material_Type T>
void FEM_Element3D<T>::compute_energy_and_derivatives(Scalar TimeStep,
                                                              ...
                                                              const Vec3& x4)
{
    ...
    ds_dp << -1, -1, -1, 1, 0, 0, 0, 1, 0, 0, 0, 1;

    // Here the B matrix is defined as the matrix to transform the coordinate system from the tetrahedron iso-parametric
    // coordinates p' = Bp + x1 Where p is a point defined in the iso-paramteric cordinate system and p' is in the same
    // reference frame as x1,x2,etc.
    Mat3 B;
    B << x2 - x1, x3 - x1, x4 - x1;

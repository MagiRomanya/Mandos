...
inline Eigen::Matrix<Scalar,3,6> compute_A_matrix(const ParticleRigidBodyCouplings& coupling, const PhysicsState& state) {
...
void Couplings::compute_rigid_body_index_conversion() {
  for (unsigned int i = 0; i < couplings.size(); i++) {
    const ParticleRigidBodyCouplings& coupling = couplings[i];
    if (rigid_body_indices_conversion.contains(coupling.rb.index)) continue;
    rigid_body_indices_conversion[coupling.rb.index] = coupling.rb.index;
    for (unsigned int j = i; j < couplings.size(); j++) {
      const ParticleRigidBodyCouplings& other = couplings[j];
  ...
  */
  ...
  for (unsigned int dof_index = 0; dof_index < nDoF; dof_index++) {
    if (couplings.dof_index_to_coupling.contains(dof_index)) {
      const ParticleRigidBodyCouplings& coupling = couplings.get_couplings(dof_index);
      if (dof_index == coupling.particle.index) {
        index_offset+=3;
        const Eigen::Matrix<Scalar, 3, 6> A = compute_A_matrix(coupling, state);
        for (unsigned int j=0; j < 3; j++) {
          for (unsigned int k=0; k < 6; k++) {
            sparse_triplets.emplace_back(dof_index + j, couplings.rigid_body_indices_conversion.at(coupling.rb.index) + k, A(j,k));
          }
        }
  ...
  SparseMat fix_particle_jacobian(nDoF, nDoF-3*couplings.couplings.size()); // Reduces the dimensionality of the problem 3 dofs per coupled particle

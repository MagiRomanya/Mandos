...
SimulableBounds generate_FEM3D_from_tetrahedron_mesh(Simulation& simulation, Scalar node_mass, Scalar poisson_ratio, Scalar young_modulus,
  ...
  for (unsigned int i = 0; i < n_particles; i++) {
    simulation.simulables.particles.emplace_back(node_mass, index + 3*i);
  }
  ...
  for (unsigned int i=0; i < n_tets; i++) {
    const Particle p1 = simulation.simulables.particles[tet_indices[4*i+0]];
    const Particle p2 = simulation.simulables.particles[tet_indices[4*i+1]];
    const Particle p3 = simulation.simulables.particles[tet_indices[4*i+2]];
    const Particle p4 = simulation.simulables.particles[tet_indices[4*i+3]];
